...
const ModeratorPanel = () => {
  ...
  // Moderadores solo ven: usuarios, reportes, apelaciones, baja
  ...
  //  \u2705 Soportar tab desde URL (para notificaciones)
  useEffect(() => {
    const tabParam = searchParams.get("tab");
    if (tabParam && ["usuarios", "reportes", "apelaciones", "baja"].includes(tabParam)) {
  ...
  useEffect(() => {
    if (loading) return;
    // \u2705 Permitir tanto MODERADOR como ADMINISTRADOR
    if (!isAuthenticated || !(user?.roles?.includes("MODERADOR") || user?.roles?.includes("ADMINISTRADOR"))) {
      navigate("/");
      return;
    }
  ...
  const loadData = async () => {
    ...
    try {
      ...
      try {
        ...
        setModeradores(Array.isArray(modData) ? modData : []);
      } catch (err) {
        console.error("Error loading moderadores:", err);
        setModeradores([]);
      }
      ...
      try {
        ...
        console.error("Error loading publicaciones config:", err);
        setPublicacionesConfig([]);
      }
      ...
      try {
        ...
        setUsuariosClientes(Array.isArray(clientesData) ? clientesData : []);
      } catch (err) {
        console.error("Error loading usuarios clientes:", err);
        setUsuariosClientes([]);
      }
      ...
      // 6. Publicaciones para dar de baja
      try {
        const publicacionesRes = await fetch(`${API}/publicaciones`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const publicacionesData = await publicacionesRes.json();
        setPublicacionesBaja(Array.isArray(publicacionesData) ? publicacionesData : []);
      } catch (err) {
        console.error("Error loading publicaciones baja:", err);
        setPublicacionesBaja([]);
      }
      ...
      try {
        ...
        setApelaciones(Array.isArray(apelacionesData) ? apelacionesData : []);
      } catch (err) {
        console.error("Error loading apelaciones:", err);
        setApelaciones([]);
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="container mx-auto py-10 px-6 flex-1">
        <h1 className="text-3xl font-bold mb-6">Panel de Moderador</h1>

        {/* TABS - Solo para moderadores */}
        <div className="flex gap-4 mb-6 border-b border-border overflow-x-auto">
          <button
            onClick={() => setActiveTab("usuarios")}
            className={`pb-2 px-4 font-semibold transition-colors ${activeTab === "usuarios"
          ...
          <button
            ...
            className={`pb-2 px-4 font-semibold transition-colors ${activeTab === "reportes"
              ? "border-b-2 border-primary text-primary"
              : "text-muted-foreground hover:text-foreground"
              }`}
          >
          ...
          <button
            ...
            className={`pb-2 px-4 font-semibold transition-colors ${activeTab === "apelaciones"
              ? "border-b-2 border-primary text-primary"
              : "text-muted-foreground hover:text-foreground"
              }`}
          >
          ...
          <button
            ...
            className={`pb-2 px-4 font-semibold transition-colors ${activeTab === "baja"
              ? "border-b-2 border-primary text-primary"
              : "text-muted-foreground hover:text-foreground"
              }`}
          >
                  ...
                  <Card key={p.id} className="overflow-hidden shadow">
                    ...
                    <CardContent className="p-4 space-y-3 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-semibold ${p.estado === "DADO_DE_BAJA"
                            ? "text-red-600"
                            : "text-green-600"
                            }`}
                        >
                  ...
                  <Card key={u.id} className="overflow-hidden">
                    ...
                    <CardContent className="space-y-3 text-xs">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-[11px] ${u.estadoCuenta === "ACTIVO"
